 ...
 *
 * A Job using the PBS is special in that way that it does not control the execution of its actual
 * task. This is completely controlled by the PBS. The Job could gain information about the state of
 * the execution of the task scheduled on the PBS, but this would increase the critical path of the
 * Job execution where it is not save to shutdown the application. After a non-clean shutdown it would
 * be impossible to know whether the Job has started the task on the PBS or not.
 *
