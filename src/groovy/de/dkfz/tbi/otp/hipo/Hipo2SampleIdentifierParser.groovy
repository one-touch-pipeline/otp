...
class Hipo2SampleIdentifierParser implements SampleIdentifierParser {
    ...
    static String REGEX = /^(?<pid>(?<project>K[0-9]{2}[A-Z])-[A-Z0-9]{6})-(?<tissueType>[${HipoTissueType.values()*.key.join('')}])(?<tissueNumber>[0-9]{1,2})-(?<analyte>[DRPAWYLTME][0-9]|[0-9][CGH][0-9]{2})$/
