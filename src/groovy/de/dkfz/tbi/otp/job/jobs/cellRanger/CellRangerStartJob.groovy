...
class CellRangerStartJob extends AbstractAlignmentStartJob implements RestartableStartJob {
    ...
    AbstractMergedBamFile reallyCreateBamFile(MergingWorkPackage mergingWorkPackage, int identifier, Set<SeqTrack> seqTracks, ConfigPerProjectAndSeqType config, AbstractMergedBamFile baseBamFile = null) {
        new SingleCellBamFile(
                ...
                workDirectoryName: "${mergingWorkPackage.expectedCells}_${mergingWorkPackage.enforcedCells}_${mergingWorkPackage.config.programVersion}",
                seqTracks: seqTracks,
        )
    }
    ...
    ConfigPerProjectAndSeqType getConfig(MergingWorkPackage mergingWorkPackage) {
        CellRangerConfig config = CollectionUtils.<CellRangerConfig> atMostOneElement(CellRangerConfig.findAllByProjectAndPipelineAndSeqTypeAndObsoleteDate(mergingWorkPackage.project, mergingWorkPackage.pipeline, mergingWorkPackage.seqType, null))
        assert config: "Could not find one CellRangerConfig for ${mergingWorkPackage.project}, ${mergingWorkPackage.seqType} and ${mergingWorkPackage.pipeline}"
        return config
    }
}
