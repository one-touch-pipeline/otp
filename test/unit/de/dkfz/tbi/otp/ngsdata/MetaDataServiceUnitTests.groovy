...
class MetaDataServiceUnitTests {
    ...
    @Test
    void testFindOutMateNumberIfSingleEndOrByFileName_SingleEnd() {
        def file = 'D0DDVABXX_lane4.fastq.gz'
        assertEquals(1, MetaDataService.findOutMateNumberIfSingleEndOrByFileName(file, true))
        assertEquals(1, MetaDataService.findOutMateNumberIfSingleEndOrByFileName(null, true))
    }

    @Test
    void testFindOutMateNumberIfSingleEndOrByFileName_FromFileName() {
        def file = 'SOMEPID_L001_R2.fastq.gz'
        assertEquals(2, MetaDataService.findOutMateNumberIfSingleEndOrByFileName(file, false))
    }
    ...
    @Test
    void testAddMateNumberFileTypeNeedsNoMateInfo() {
        ...
        assert dataFile.mateNumber == null
        metaDataService.addMateNumber(dataFile, fileType.type)
        assert dataFile.mateNumber == null
    }

    @Test
    void testAddMateNumberLibraryIsSingleAndFastqSequenceType() {
        ...
        metaDataService.addMateNumber(dataFile, fileType.type)
        assert dataFile.mateNumber == 1
    }

    @Test
    void testAddMateNumberLibraryIsPairedAndFastqSequenceType() {
        FileType fileType = FileType.build(type: FileType.Type.SEQUENCE, signature: "fastq", vbpPath: FASTQ_SEQUENCE_DIRECTORY)
        DataFile dataFile = createDataFileAndMetadataEntry("s_101202_7_2.fastq.gz", SeqType.LIBRARYLAYOUT_PAIRED)

        metaDataService.addMateNumber(dataFile, fileType.type)
        assert dataFile.mateNumber == 2
    }

    @Test
    void testAddMateNumberLibraryIsSingleAndNotFastqSequenceType() {
    ...
    @Test
    void testAddMateNumberLibraryIsPairedAndNotFastqSequenceType() {
        FileType fileType = FileType.build(type: FileType.Type.SEQUENCE, signature: "fastq", vbpPath: NO_FASTQC_SEQUENCE_DIRECTORY)
        DataFile dataFile = createDataFileAndMetadataEntry("s_101202_7_2.fastq.gz", SeqType.LIBRARYLAYOUT_PAIRED)
