 ...
 * To have a clear structure of the config files it was decided to have one config file per project and pipeline.
...
class RoddyWorkflowConfig extends ConfigPerProject {
    ...
    static constraints = {
        ...
        pipeline unique: ['project', 'configFilePath']
        pluginVersion blank: false
        obsoleteDate unique: ['project', 'pipeline']  // partial index: WHERE obsolete_date IS NULL
        configVersion nullable: true, blank: false, unique: ['project', 'pipeline', 'pluginVersion'], matches: CONFIG_VERSION_PATTERN //needs to be nullable because of old data
    }

    static void importProjectConfigFile(Project project, String pluginVersionToUse, Pipeline pipeline, String configFilePath, String configVersion) {
        ...
        RoddyWorkflowConfig roddyWorkflowConfig = getLatest(project, pipeline)
    ...
    static RoddyWorkflowConfig getLatest(final Project project, final Pipeline pipeline) {
        assert project : "The project is not allowed to be null"
        assert pipeline : "The pipeline is not allowed to be null"
        try {
            return atMostOneElement(RoddyWorkflowConfig.findAllByProjectAndPipelineAndObsoleteDate(project, pipeline, null))
    ...
    void validateConfig() {
        ...
        String pattern = /^${Pattern.quote(pipeline.name.name())}_(.+)_${Pattern.quote(configVersion)}\.xml$/
    ...
    String getNameUsedInConfig() {
        String nameInConfigFile = "${pipeline.name}_${pluginVersion}"
        if (configVersion) {
