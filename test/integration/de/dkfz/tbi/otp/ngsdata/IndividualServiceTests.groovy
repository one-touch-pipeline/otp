...
class IndividualServiceTests extends AbstractIntegrationTest {
    ...
    @Test
    void testPreviousNextIndividual() {
        // everything as admin - security is checked in another test
        SpringSecurityUtils.doWithAuth("admin") {
            Individual individual = mockIndividual()
            // there should be neither a previous nor a next individual
            ...
            Individual individual1 = mockIndividual("test1")
            // now next from individual should work
            assertNull(individualService.previousIndividual(individual))
            assertSame(individual1, individualService.nextIndividual(individual))
            // and prev from individual1 should work
            assertSame(individual, individualService.previousIndividual(individual1))
            assertNull(individualService.nextIndividual(individual1))

            // adding a third Individual
            Individual individual2 = mockIndividual("test2")
            // that should not have affected our first individual
            assertNull(individualService.previousIndividual(individual))
            assertSame(individual1, individualService.nextIndividual(individual))
            // now from individual1 navigation in both directions should work
            assertSame(individual, individualService.previousIndividual(individual1))
            assertSame(individual2, individualService.nextIndividual(individual1))
            // and from last individual we should only have previous
            assertSame(individual1, individualService.previousIndividual(individual2))
            assertNull(individualService.nextIndividual(individual2))
        }
    }

    @Test
    void testPreviousIndividualSecurity() {
        Individual individual = mockIndividual()
        // a user should not be allowed to access the previous individual for our individual
        SpringSecurityUtils.doWithAuth("testuser") {
            shouldFail(AccessDeniedException) {
        ...
        // now the testuser should be able to access the method for our individual
        SpringSecurityUtils.doWithAuth("testuser") {
            assertNull(individualService.previousIndividual(individual))
        }
    ...
    @Test
    void testNextIndividualSecurity() {
        Individual individual = mockIndividual()
        // a user should not be allowed to access the next individual for our individual
        SpringSecurityUtils.doWithAuth("testuser") {
            shouldFail(AccessDeniedException) {
