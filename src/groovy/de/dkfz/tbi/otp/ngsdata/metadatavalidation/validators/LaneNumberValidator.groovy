...
class LaneNumberValidator extends SingleValueValidator<MetadataValidationContext> implements MetadataValidator {
    ...
    void validateValue(MetadataValidationContext context, String laneNumber, Set<Cell> cells) {
        if (laneNumber.empty) {
            ...
            context.addProblem(cells, Level.ERROR, "'${laneNumber}' is not a well-formed lane number. It must contain only digits (0 to 9) and/or letters (a to z, A to Z). It should be a single digit in the range from 1 to 8.")
        } else if (!(laneNumber ==~ /^[1-8]$/)) {
            context.addProblem(cells, Level.WARNING, "'${laneNumber}' is not a well-formed lane number. It should be a single digit in the range from 1 to 8.")
        }
    }
}
