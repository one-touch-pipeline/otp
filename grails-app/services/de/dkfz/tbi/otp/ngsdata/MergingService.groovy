...
class MergingService {
    ...
    private void buildSeqScan(Sample sample, SeqType seqType, 
        ...
        DataFile file = 
            DataFile.findByFileNameAndPathName(fileName, pathToBam)

        if (file != null) {
            println "File ${fileName} in ${pathToBam} already registered"
            return
        }
        ...
        if (!bamFile.canRead()) {
            println "Can not read file ${bamFile}"
            return
        }

        List<String> header = parseBamHeader(bamFile)
        if (header == null) {
             println "No hader for file ${bamFile}"
             return
        }
    ...
    private SeqScan getMatchingSeqScan(List<SeqScan> seqScans, List<SeqTrack> tracks) {
        ...
        for(int i=0; i<seqScans.size(); i++) {
            ...
            if (scan.seqTracks.size() != tracks.size()) {
                println "size does not match ${scan.seqTracks.size()} ${tracks.size()}"
                continue
            }
            boolean match = true
     ...
     * @return
     */
    private List<SeqTrack> getSeqTracks(Sample sample, 
        SeqType seqType, List<String> header) {
     ...
     * @return
     */
    private SeqTrack getSeqTrack(Sample sample, SeqType seqType,
             String runName, String lane) {

        String runString = runName.substring(3) // removing "run"
        //int laneId = lane as int

        //println "${sample} ${seqType} ${runName} ${laneId}"
        ...
        if (seqTracks.size() != 1) {
            println "Number of seqTracks = ${seqTracks.size()}"
        } else {
    ...
    private List<SeqScan> getSeqScans(Sample sample, SeqType seqType) {
        ...
        List<SeqScan> scans = 
            SeqScan.findAllBySampleAndSeqType(sample, seqType)
        return scans
    }
    ////////////////////////////////////////////////////////////////////////////
    ...
    private AlignmentParams getAlignmentParamsFromHeader(File bamFile) {
        ...
        AlignmentParams alignParams =
            AlignmentParams.findByProgramNameAndProgramVersion(progName, progVersion)
