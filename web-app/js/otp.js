...
$.otp = {
    ...
    /**
     * Helper method to extend the given link by a further component. Ensures
     * that there is exactly one slash between the link and the further
     * component.
     * 
     * @param link
     *            The original link
     ...
     * 
     * @param options
     *            The URL parts to construct the link from
    ...
    /**
     * Creates the HTML markup for an a element from the passed in options. For
     * the actual link (href) the same attributes in options are supported as in
     * {@link $.otp.createLink}. In addition the following attributes in
     ...
     * 
     * @param options
     *            The options defining the hyperlink
...
/**
 * Generates a generic datatable. The datatable expects an ajax source at
 * "dataTableSource" which provides a JSON response. The first column of the
 * data source is expected to be a complex datatabe consisting of <em>id</em>
 * and a descriptive <em>text</em>.
 * 
 * @param selector
 *            The jQuery selector of the table
 * @param showLink
 *            The link to the controller/action to show more details of an
 *            element
 */
...
$.otp.sequence = {
    ...
    register : function() {
                ...
                .dataTable(
                        {
                            ...
                            bScrollCollapse : true,
                            iDisplayLength : Math
                                    .round(($('.body').height() - 200) / 23),
                            bDeferRender : true,
                            fnServerData : function(sSource, aoData, fnCallback) {
                                aoData.push({
                                    name : "filtering",
                                    value : JSON.stringify($.otp.sequence
                                            .searchCriteria())
                                });
                                $
                                        .ajax({
                                                        ...
                                                        for (j = 0; j < row.fastQCFiles.length; j += 1) {
                                                            fastQC += $.otp
                                                                    .createLinkMarkup({
                                                                        ...
                                                                        text : $.i18n
                                                                                .prop(
                                                                                        "sequence.list.numberedFastQCFile",
                                                                                        (j + 1))
                                                                    });
                                                            fastQC += " ";
                                                        }
        ...
        // search criteria
        $("#searchCriteriaTable tr td:eq(0) select").change(
                $.otp.sequence.searchCriteriaChangeHandler);
        $("#searchCriteriaTable tr td:eq(2) input[type=button]").click(
                $.otp.sequence.searchCriteriaAddRow);
        $("#searchCriteriaTable tr td:eq(1) select").change(
                $.otp.sequence.updateSearchCriteria);
        $("#searchCriteriaTable tr td:eq(1) input[type=text]").change(
                $.otp.sequence.updateSearchCriteria);
        $("#searchCriteriaTable tr td:eq(1) input[type=text]").keydown(
                $.otp.sequence.updateSearchCriteria);
    },
    ...
    searchCriteriaAddRow : function() {
        ...
        $("td:eq(0) select", cloned).change(
                $.otp.sequence.searchCriteriaChangeHandler);
        $("td:eq(2) input[type=button]", cloned).click(
                $.otp.sequence.searchCriteriaAddRow);
        $("td:eq(1) select", cloned)
                .change($.otp.sequence.updateSearchCriteria);
        $("td:eq(1) input[type=text]", cloned).change(
                $.otp.sequence.updateSearchCriteria);
        $("td:eq(1) input[type=text]", cloned).keydown(
                $.otp.sequence.updateSearchCriteria);
    },
    searchCriteria : function() {
        "use strict";
        var result = [];
        $("#searchCriteriaTable tr").each(
...
$.otp.option = {
    formatValue : function(value) {
        ...
        if (value.length > MAX_SIZE) {
            result = "..."
                    + value.substr(value.length - MAX_SIZE - 1, value.length);
        } else {
            result = value;
        }
        return result;
...
$.otp.addIndividual = {
    addSample : function(event) {
        ...
        sampleBoxes.find("button").click(
                $.otp.addIndividual.addSampleIdentifier);
                ...
                function(data) {
                    ...
                    if (data.error) {
                        ...
                        for (i = 0; i < data.errors.length; i += 1) {
                            message += "<li>" + data.errors[i].message
                                    + "</li>";
                        }
                        message += "</ul>";
                        $.otp.warningMessage(message);
                    } else if (data.success) {
                        $.otp.infoMessage($.i18n.prop(
                                "individual.insert.add.success", data.id));
                    }
 ...
 * 
 * @param selector
 *            The JQuery selector for the table to create the datatable into
 * @param sourcePath
 *            The path to the ajax resource
 * @param sortOrder
 *            {@code true} for ascending, {@code false} for descending initial
 *            sorting of first collumn
 * @param jsonCallback
 *            (optional) the callback to invoke when json data has been
 *            returned. Get's one argument json
 * @param columnDefs
 *            (optional) Array of column definitions, can be used to
 *            enable/disable sorting of columns
 * @param postData
 *            (optional) Array of additional data to be added to the POST
 *            requests
 */
$.otp.createListView = function(selector, sourcePath, sortOrder, jsonCallback,
        columnDefs, postData) {
    "use strict";
    $(selector).dataTable({
        bFilter : false,
...
$.otp.workFlow = function(selector, sourcePath, sortOrder, jsonCallback,
        columnDefs, postData) {
    "use strict";
    $(selector).dataTable({
        bFilter : false,
...
$.otp.registerStep = function(selector, sourcePath, sortOrder, jsonCallback,
        columnDefs, postData) {
    "use strict";
    $(selector).dataTable({
        bFilter : false,
...
$.otp.autorefresh = {
    ...
    /**
     * The click handler for the links. Performs an AJAX request to
...
$.otp.notificationAdministration = {
                ...
                .dataTable(
                        {
                                        ...
                                        .ajax({
                                            ...
                                            "success" : function(json) {
                                                ...
                                                for (i = 0; i < json.aaData.length; i += 1) {
                                                    ...
                                                    trigger += '<input type="hidden" name="id" value="'
                                                            + row.trigger.id
                                                            + '"/>';
                                                    trigger += '<input type="hidden" name="className" value="'
                                                            + row.trigger.clazz
                                                            + '"/>';
                                                    trigger += '<input type="hidden" name="triggerId" value="'
                                                            + row.trigger.triggerId
                                                            + '"/>';
                                                    rowData = [
                                                            '<input type="checkbox" name="'
                                                                    + row.notification.id
                                                                    + '" '
                                                                    + (row.notification.enabled ? 'checked="checked"'
                                                                            : '')
                                                                    + '/>',
                            ...
                            fnRowCallback : function(nRow) {
                                ...
                                notificationId = $(
                                        "td:eq(0) input[type=checkbox]", nRow)
                                        .attr("name");
                                $("td:eq(0) input[type=checkbox]", nRow)
                                        .click(
                                                $.otp.notificationAdministration.enableNotification);
                                // set the Type select
                                cell = $('td:eq(1)', nRow);
                                selection = $("#typeSelection-Prototype")
                                        .clone();
                                        ...
                                        .change(
                                                function() {
                                                    ...
                                                    $
                                                            ...
                                                            .error(
                                                                    $.otp.notificationAdministration.errorHandler);
                                                });
                                // set the medium select
                                cell = $('td:eq(2)', nRow);
                                selection = $("#mediumSelection-Protoype")
                                        .clone();
                                $("option[value=" + cell.text() + "]",
                                        selection).attr("selected", true);
                                selection.removeAttr("id");
                                ...
                                $('<button class="edit"></button>')
                                        ...
                                        .click(
                                                function() {
                                                    ...
                                                    $("input:hidden", dialog)
                                                            .val(
                                                                    $(
                                                                            ...
                                                                            .val());
                                                    if ($(
                                                            "input:hidden[name=className]",
                                                            cell).val() === "de.dkfz.tbi.otp.job.plan.JobExecutionPlan") {
                                                        // it's a job execution
                                                        // plan
                                                        $(
                                                                "select[name=jobExecutionPlan] option[value="
                                                                        + triggerId
                                                                        + "]",
                                                                dialog).attr(
                                                                "selected",
                                                            ...
                                                            "input:hidden[name=className]",
                                                            cell).val() === "de.dkfz.tbi.otp.job.plan.JobDefinition") {
                                                        $
                                                                .getJSON(
                                                                        $.otp
                                                                                .createLink({
                                                    ...
                                                    // when the checkbox get
                                                    // checked load the
                                                    // JobDefinitions
                                                    $("input:checkbox", dialog)
                                                            .click(
                                                                    function() {
                                                                        var jobSelection = $(
                                                                                "select[name=jobDefinition]",
                                                                                    ...
                                                                                    .getJSON(
                                                                                            $.otp
                                                                                                    .createLink({
                                                                                                        ...
                                                                                                        id : $(
                                                                                                                "select[name=jobExecutionPlan] option:selected",
                                                                                                                dialog)
                                                                                                                .val()
                                                                                                    }),
                                                                                            function(
                                                    ...
                                                    // when the workflow
                                                    // selection changes, reload
                                                    // the job selection (if
                                                    // shown)
                                                    $(
                                                                                ...
                                                                                .getJSON(
                                                                                                ...
                                                                                                }),
                                                                                        {
                                                                                            jobExecutionPlan : $(
                                                                                                    ...
                                                                                                    .val(),
                                                                                            jobDefinition : $(
                                                                                                    "select[name=jobDefinition] option:selected",
                                                                                                    dialog)
                                                                                                    .val()
                                                                                        },
                                                                                        $.otp.notificationAdministration.notificationUpdate)
                                ...
                                $.otp.notificationAdministration.template($(
                                        'td:eq(4)', nRow));
                                $.otp.notificationAdministration.template($(
                                        'td:eq(5)', nRow));
                                return nRow;
     ...
     * 
     * @param jobSelection
     *            jQuery object pointing to a selection list
     * @param jobs
     *            List of objects with at least elments id and name
     */
    createJobOptions : function(jobSelection, jobs) {
        ...
        for (i = 0; i < jobs.length; i += 1) {
            ...
            $('<option value="' + job.id + '">' + job.name + '</option>')
                    .appendTo(jobSelection);
        }
    },
    /**
     * Creates the cell markup for a template. The text is shortened with an
     * expand link and an edit button is added which shows a dialog to update
     * the template.
     * 
     * @param cell
     *            The table cell the template is in
     */
    ...
    /**
     * Adjusts the template for too long text by using only the first twenty
     * characters and adding a link to expand the text.
     * 
     * @param span
     *            The text element to shorten
     */
    expandTemplateMarkup : function(span) {
        "use strict";
        span.text(span.text().substring(0, 19));
        $('<a href="#">…</a>').appendTo(span).click(
                $.otp.notificationAdministration.expandTemplate);
    },
    /**
     * Callback for expanding the text of a template. Reads the actual text from
     * the title element of the parent of the clicked element and sets it on
     * this element. In addition a link to collapse the text again is added.
     * 
     * @param event
     *            The click event which triggered the expand
     */
    expandTemplate : function(event) {
        ...
        $(
                '<a href="#">'
                        + $.i18n
                                .prop("notification.administration.template.collapse")
                        + '</a>').appendTo(span).click(
                $.otp.notificationAdministration.collapseTemplate);
    },
    /**
     * Callback for collapsing the text of a template. Shortens in the same way
     * as in the initial rendering of the table.
     * 
     * @param event
     *            The click event which triggered the collapse
     */
    ...
    /**
     * Callback for the enable/disable notification checkbox. Performs an AJAX
     * request to update the enable state of the notification.
     */
    enableNotification : function() {
        ...
        }, $.otp.notificationAdministration.notificationUpdate).error(
                $.otp.notificationAdministration.errorHandler);
    },
    /**
     ...
     * It expects a JSON structure containing either a boolean success field or
     * an error with the error message. In success case a notification is shown
     * informing about the update, in error case the error message is shown. In
     * both cases the datatable is reloaded to show the current state.
     * 
     * @param data
     *            The JSON data fetched through AJAX
     */
    notificationUpdate : function(data) {
        "use strict";
        var i, message;
        if (data.success) {
            $.otp.infoMessage($.i18n
                    .prop("notification.administration.update.success"));
        } else if (data.error) {
            $.otp.warningMessage(data.error);
        } else if (data.errors) {
            message = "<ul>";
    ...
    errorHandler : function(jqXHR) {
        "use strict";
        if (jqXHR.status === 404) {
            $.otp.warningMessage($.i18n
                    .prop("notification.administration.update.notFound"));
        } else {
            $.otp.warningMessage(jqXHR.statusText + jqXHR.status);
        }
    }
        ...
        $.getJSON(
                ...
                function(data) {
                    ...
                    if (data.error) {
                        ...
                        $.otp
                                .infoMessage($.i18n.prop(
                                        "processingOption.insert.add.success",
                                        data.id));
                    }
                }).error(function(jqXHR) {
            $.otp.warningMessage(jqXHR.statusText + jqXHR.status);
...
$.otp.highlight = function(path) {
    ...
    if (pathSplit.length > 3) {
        ...
        $('.menuContainer #' + pathSplit[2] + ' a').attr('style',
                'color: #fafafa;');
        if (pathSplit[2] == "overviewMB" || pathSplit[2] == "projectOverview"
                || pathSplit[2] == "projectStatistic") {
            $('.menuContainer #overview a:first').attr('style',
                    'color: #fafafa;');
        } else if (pathSplit[2] == "userAdministration"
                || pathSplit[2] == "group" || pathSplit[2] == "crashRecovery"
                || pathSplit[2] == "shutdown" || pathSplit[2] == "notification"
                || pathSplit[2] == "processingOption"
                || pathSplit[2] == "softwareTool") {
...
$.otp.projectOverviewTable = {
    registerProjectOverview : function() {
        "use strict";
        var oTable = $("#projectOverviewTable")
                .dataTable(
                        {
                            sDom : 'T<"clear">lfrtip',
                            oTableTools : {
    ...
    registerProjectOverviewSequnceTypeTable : function() {
        "use strict";
        var oTable2 = $("#patientsAndSamplesGBCountPerProject")
                .dataTable(
                        {
                            sDom : 'T<"clear">lfrtip',
                            oTableTools : {
    ...
    registerSampleTypeNameCountBySample : function() {
        "use strict";
        var oTable4 = $("#sampleTypeNameCountBySample")
                .dataTable(
                        {
                            sDom : 'T<"clear">lfrtip',
                            oTableTools : {
...
$.otp.createListViewProcessingStep = function(selector, sourcePath, sortOrder,
        jsonCallback, columnDefs, postData) {
    "use strict";
    $(selector).dataTable({
        bFilter : false,
        bJQueryUI : false,
...
$.otp.resizeBodyInit = function(table, margin) {
    $(window).resize(
            function() {
                $(table + '_wrapper' + ' .dataTables_scrollBody').height(
                        ($('.body').height() - margin));
            })
}
